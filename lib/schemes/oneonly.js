...
function OneOnlyScheme (steelmesh, watchdog, app) {    
    ...
    // Handle application updates (stop the currently running instance)
    meshEvents.on('app.update.' + this.app.id, function(callback) {
        log('Application update');
        scheme.stop(callback);
    });
    
    meshEvents.on('app.load.' + this.app.id, function(app) {
