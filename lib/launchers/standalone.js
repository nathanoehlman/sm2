...
/**
  Sends messages to the child
 **/
StandaloneApplication.prototype.send = function() {
    if (this.child) {
        log('sending message');
        this.child.send.call(this.child, arguments);
    }
}
...
StandaloneApplication.prototype._handleMessage = function(message, handle) {    
    this.emit('message.' + (message.operation || 'unknown'), message, handle);    
}
