...
SteelmeshServer.prototype.startDash = function(callback) {
    callback();
};

SteelmeshServer.prototype.startMonitor = function(callback) {
    callback();
};
...
module.exports = function(opts) {
    ...
    async.series([
        ...
        // start the dashboard
        server.startDash.bind(server),
        
        // load the apps
        server.loadApps.bind(server),
        
        // start the monitor
        server.startMonitor.bind(server)
    ], function(err) {
        if (err) {
            out('!{red}{0}', err);
            process.exit();
